...
- **Host environment for phases A–D.** x86-64, `python3` stdlib, a host cc with ASan/UBSan, and `binutils-aarch64-linux-gnu`. No cross compiler and no emulator are available or required. The cross *assembler* is required by `make check`; the build says so by name rather than skipping the check.
...
- **qemu-user execution of every variant.** Ruled out by the human, and the disassembly re-parse gate (SPEC §6.4.5) covers most of what it would catch. It would additionally catch AAPCS clobber violations and stack corruption that neither the model nor the disassembler notices. `qemu-user-static` is a single package if that gap ever matters. qemu does not model Device-memory alignment faults, so it would never be the alignment gate.
